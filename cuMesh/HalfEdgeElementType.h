...
/**
 * VFMeshData class to store the mesh data
 *
 * @author jothepro
 *
 */
struct VFMeshData {
    ...
    std::vector<std::array<double, 3>> GetVertexPositions() const{
        ...
        for (int i = 0; i < vertices.size(); i++) positions[i] = {vertices[i].x, vertices[i].y, vertices[i].z};
        return positions;
    }
     ...
     * @return
     */
    std::vector<std::vector<size_t>> GetTriangleIndices() const {
