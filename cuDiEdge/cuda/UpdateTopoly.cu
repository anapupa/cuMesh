#include <cuDiEdge/cuda/HalfEdgeElementType.cuh>

#include "UpdateTopology.cuh"
#include <thrust/scan.h>

constexpr static int blk_size = 32;
...
namespace cuMesh {
    void Topology::UpdateFaceFace(VFMeshData &mesh_data) {
        mesh_data._hedges_twin.resize(mesh_data._facets.size());
        {
            ...
            // 1.3 fill vertex's outgoing hedges
            thrust::device_vector<__half> v_global(mesh_data._vertices.size(), 0);
        ...
        //step1: count each vertex's degree

        //step2: accumulate v_degree as offseted array's index
